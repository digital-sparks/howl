...
async function waitForMixpanel(timeout = 1000, retries = 100, delay = 100) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    if (attempt < retries) {
      console.warn(`Mispanel not ready (attempt ${attempt}/${retries}). Retrying in ${delay}ms...`);
...
$('#save-specialty').click(function () {
  ...
  if (selectedSpecialties && selectedSpecialties.length > 0) {
    ...
    $('#specialties-label-nav').text('Any needs?(optional)').removeClass('is-active');
  }
...
const searchInputs = document.querySelectorAll('.br__insurance-search-input, .input-search-popups');
  ...
  input.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Enter') {
      ...
      if (inputSearchListActiveIndex >= 0) {
        ...
        if (
          selectedItem.querySelector("input[type='checkbox'], input[type='radio']").type === 'radio'
        ) {
