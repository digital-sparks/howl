...
let stateAlert = document.getElementById("state-error");
let insuranceAlert = document.getElementById("insurance-error");
let heroAlertScroll = document.getElementById("hero-alert-scroll");
let stateAlertScroll = document.getElementById("state-error-scroll");
let insuranceAlertScroll = document.getElementById("insurance-error-scroll");
...
const herofilterFieldElements = herofilterSection.querySelectorAll(
  ".br__search-field-wr"
);

const navfilterSection = document.querySelector(".br__header_container");
const navFilterFieldElements = navfilterSection.querySelectorAll(
  ".br__search-field-wr"
);
...
function toggleDropdown(event) {
  ...
  var clickedDropdown = event.currentTarget.parentElement.querySelector(
    ".hero_dropdown-wrap"
  );
  ...
  for (var i = 0; i < dropdownElements.length; i++) {
    ...
    if(clickedDropdown && ["insurance-dropdown", "insurance-dropdown-nav", "specialty-dropdown", "specialty-dropdown-nav"].includes(clickedDropdown.id)) {
  ...
  setTimeout(() => {
    updateKeyboardHighlight(
      clickedDropdown?.querySelectorAll(".w-dyn-item") ?? []
    );
  }, 10);
}
...
function closeDropdownsOutsideForm(event) {
  ...
  if (!isDropdownClicked) {
    ...
    for (var i = 0; i < dropdownElements.length; i++) {
      ...
      const dropdownItems =
        dropdownElements[i]?.querySelectorAll(".w-dyn-item");
      setTimeout(() => {
...
const insuranceLabelNav = document.getElementById("insurance-label-nav");
const insuranceLabelMobileScroll = document.getElementById(
  "insurance-label-scroll"
);
...
function validateStateField() {
  ...
  if (selectedState.state !== undefined) {
    stateAlert.classList.add("is-hidden");
    stateAlertScroll.classList.add("is-hidden");
    heroAlert.classList.add("is-hidden");
  } else {
    heroAlert.classList.remove("is-hidden");
    stateAlertScroll.classList.remove("is-hidden");
    stateAlert.classList.remove("is-hidden");
  }
}

function validateInsuranceField() {
  if (selectedInsurance != null) {
    insuranceAlert.classList.add("is-hidden");
    insuranceAlertScroll.classList.add("is-hidden");
  } else {
    heroAlert.classList.remove("is-hidden");
    heroAlertScroll.classList.remove("is-hidden");
    insuranceAlert.classList.remove("is-hidden");
    insuranceAlertScroll.classList.remove("is-hidden");
  }
}
...
$("#save-specialty").click(function () {
  ...
  if (selectedSpecialties && selectedSpecialties.length > 0) {
    ...
    $("#specialties-label-scroll")
      .text(selectedValues.join(", "))
      .addClass("is-active");

    $("#specialties-label-nav")
      .text(selectedValues.join(", "))
      .addClass("is-active");
  } else {
    $("#specialties-label")
      .text("Any needs?(optional)")
  ...
  setTimeout(() => {
    specialtyDropdown.hasClass("is-active")
      ? specialtyDropdown.removeClass("is-active")
      : null;
...
$("#save-specialty-nav").click(function () {
  var checkboxes = $(
    '#specialty-dropdown-nav .filter-item input[type="checkbox"]'
  );
  selectedSpecialties = [];
  var selectedValues = [];
...
$("#specialties .save").click(function () {
  var checkboxes = $(
    '#specialty-collection-mobile .filter-item input[type="checkbox"]'
  );
  selectedSpecialties = [];
  var selectedValues = [];
...
function storeInputText() {
  if (selectedInsurance == null || selectedState.state === undefined) {
    ...
    heroAlert.classList.add("is-hidden");
    heroAlertScroll.classList.add("is-hidden");
    insuranceAlert.classList.add("is-hidden");
    insuranceAlertScroll.classList.add("is-hidden");
    stateAlert.classList.add("is-hidden");
    stateAlertScroll.classList.add("is-hidden");
...
// All insurance items
const insuranceItems = document.querySelectorAll(
  ".insurance-wrapper .insurance-item"
);
...
function resetInsuranceInputSearch() {
  ...
  if (insuranceWrapper)
    insuranceWrapper.classList.remove("is-searching-active");
...
specialtySearchInputs.forEach((input) => {
  input.addEventListener("input", function () {
    ...
    const specialitiesSearchMobile = document.getElementById(
      "specialitiesSearchMobile"
    );
    if (specialitiesSearchMobile)
      specialitiesSearchMobile.valaue = this.value.trim();
...
function resetSpecialtiesInputSearch() {
  ...
  if (specialtyWrapper)
    specialtyWrapper.classList.remove("is-searching-active");
...
function clearSpecialties(event) {
  ...
  $("#specialties-label").text("Any needs?(optional)").removeClass("is-active");
...
function resetSpecialtyHeroSearch() {
  ...
  if (selectedSpecialties && selectedSpecialties.length > 0) {
    $("#specialties-label").text(
      Object.values(selectedSpecialtiesText).join(", ")
    );
    $("#specialties-label").addClass("is-active");

    $("#specialties-label-scroll").text(
      Object.values(selectedSpecialtiesText).join(", ")
    );
    $("#specialties-label-scroll").addClass("is-active");

    $("#specialties-label-nav").text(
      Object.values(selectedSpecialtiesText).join(", ")
    );
    $("#specialties-label-nav").addClass("is-active");
  } else {
    $("#specialties-label").text("Any needs?(optional)");
    ...
    $("#specialties-label-nav").text("Any needs?(optional)");
...
function preFillHeroFilterAfterBack() {
  try {
    ...
    if (filterValue?.selectedInsurance) {
      ...
      $(
        "#insurance-dropdown .insurance-item, #insurance-dropdown-nav .insurance-item"
      ).each((index, element) => {
  ...
  input.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowDown") {
      ...
      inputSearchListActiveIndex =
        (inputSearchListActiveIndex + 1) % visibleItems.length;
...
function updateKeyboardHighlight(visibleItems) {
  visibleItems.forEach((item, idx) => {
    if (idx === inputSearchListActiveIndex)
