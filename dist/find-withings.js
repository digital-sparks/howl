...
const withingsToken = urlParamsFind1.get('withings_token') ?? urlParamsFind1.get('withingsToken');
...
const pcrit = (urlParamsFind1.get('pcrit') ?? localStorage.getItem('pcrit')) || null;

if (has_withings_plus == 'true') {
  $('.with-flow-hint').removeClass('hide');
}

// Hit impression endpoint
// Read existing called tokens from sessionStorage (or empty object if none)
let withingsImpressionCalled = JSON.parse(sessionStorage.getItem('withingsImpressionCalled')) || {};
// Check if required variables exist and this token hasn't been used yet
if (withingsToken && has_withings_plus != null && !withingsImpressionCalled[withingsToken]) {
...
var togElements = document.querySelectorAll('.find-w-params-wr .navbar_filter-toggle');
var dropdownElements = document.querySelectorAll('.find-w-params-wr .hero_dropdown-wrap');
...
function closeDropdownsOutsideForm(event) {
  ...
  var isDropdownClicked =
    clickedElement.closest('.filter-wrapper:not(.search-location-element)') !== null;
...
function openPopup(popup, trigger) {
  ...
  if (popup.getAttribute('id') == 'location-popup' || popup.getAttribute('id') == 'location') {
    ...
    if (isUrlHasParams) {
      ...
      if (state) {
        ...
        if (stateText) {
          ...
          tempSelectedState.stateInputText = inputText.stateText ? inputText.stateText : '';
        } else {
...
const resultDropdownElements = document.querySelectorAll('.find-data-wr .filter-dropdown_list');

function closeResultDropdownsOutsideForm(event) {
  var clickedElement = event.target;
  var isDropdownClicked = clickedElement.closest('.filter-dropdown_list') !== null;
  if (!isDropdownClicked) {
    for (var i = 0; i < resultDropdownElements.length; i++) {
      if (
        ...
        $('.find-data-wr .filter-dropdown_list.is-specialties .save').trigger('click');
      } else if (
...
const isOnlyStateSelected = (place) => {
  ...
  const hasState = components.some((comp) => comp.types.includes('administrative_area_level_1'));
...
$('.find-data-wr .filter-dropdown_list.is-insurance .filter_radio-button-field').change(
  function (e) {
...
function clearInsurance(refresh = true) {
  ...
  $('.filter_tag-template.is-insurance').text('Insurance').removeClass('is-active');
...
$('.find-data-wr .filter-dropdown_list.is-specialties .save').click(function (e) {
  e.preventDefault();
  const selectedSpecialties = [];
  $('.find-data-wr .filter-dropdown_list.is-specialties .w-dyn-item input[type="checkbox"]').each(
    (index, checkboxInput) => {
  ...
  $('.filter_tag-template.is-specialties').toggleClass('is-active', selectedCount > 0);
  ...
  const isSameParams =
    ...
    currentParams.every((val) => Object.keys(selectedSpecialties).includes(val));
...
$('.find-data-wr #specialties .save').click(function (e) {
  ...
  const selectedSpecialties = [];
  $('#specialty-collection .w-dyn-item input[type="checkbox"]').each((index, checkboxInput) => {
    const isChecked = $(checkboxInput).prop('checked');
    if (isChecked) {
      const specialtyText = $(checkboxInput)
        .closest('.filter_checkbox-field')
...
function clearSpecialties(refresh = true) {
  ...
  $('.specialties-filter').find('.filter_tag-template.is-specialties').removeClass('is-active');
  $('#specialty-collection').find('.filter_tag-template.is-specialties').removeClass('is-active');
  ...
  $('.specialties-filter').find("input[type='checkbox'][name='checkbox-3']").prop('checked', false);
...
$('.filter-dropdown_list.is-modalities .save').click(function (e) {
  ...
  $('.find-data-wr .filter-dropdown_list.is-modalities .w-dyn-item input[type="checkbox"]').each(
    (index, checkboxInput) => {
      const isChecked = $(checkboxInput).prop('checked');
      if (isChecked) {
  ...
  $('.filter_tag-template.is-modalities').toggleClass('is-active', selectedCount > 0);
...
$('#all .save').click(function (e) {
  e.preventDefault();
  const selectedModalities = [];
  $('#modality-collection .w-dyn-item input[type="checkbox"]').each((index, checkboxInput) => {
    const isChecked = $(checkboxInput).prop('checked');
    if (isChecked) {
      const modalityText = $(checkboxInput)
        .closest('.filter_checkbox-field')
  ...
  const selectedAvailability = document.querySelector('input[name="type"]:checked');
  ...
  let sort_order = null;
  const selectedSort = $('.filter-dropdown_list.is-sort').find('.filter-dropdown_radio.is-active');
  ...
  const isSameModalities =
    ...
    currentModalitiesParams.every((val) => Object.keys(selectedModalities).includes(val));
  ...
  const isSameParams = isSameModalities && isSameMiles && isSameAvailability && isSameSortOrder;
...
function clearModalities(refresh = true) {
  ...
  $('.modalities-filter').find('.filter_tag-template.is-modalities').removeClass('is-active');
  $('#modality-collection').find('.filter_tag-template.is-modalities').removeClass('is-active');

  $('#modality-collection .w-dyn-item input[type="checkbox"]:checked').prop('checked', false);
  $('#modality-collection').find('.filter_checkbox').removeClass('w--redirected-checked');

  $('.filter-dropdown_list.is-modalities .w-dyn-item input[type="checkbox"]:checked').prop(
    'checked',
    false
  );
...
function saveDistance() {
  ...
  const selectedDistanceValue = selectedDistance ? selectedDistance.val() : null;
...
const clearDrivingDistanceFilter = document.getElementById('clear-more-filters-1');
...
function clearSort() {
  ...
  $('.filter-dropdown_list.is-sort').find('.filter-dropdown_radio').removeClass('is-active');
}
...
function setupSortListeners() {
  $('.filter-dropdown_list.is-sort')
    ...
    .click(function (e) {
      ...
      $('.filter-dropdown_list.is-sort .filter-dropdown_radio').removeClass('is-active');
      $(this).addClass('is-active');
    ...
    radio.addEventListener('change', function (e) {
      ...
      $('input[name="type"], input[name="type-mobile"]').each(function (i, rb) {
        if ($(rb).val().trim() === value) {
          ...
          $(rb).closest('.filter_radio-button-field:not(.is-mobile)').addClass('is-active');
      ...
      $('.filter-dropdown_list.is-visittype').closest('.find-dropdown').trigger('w-close');
...
async function fetchProviderProfiles() {
  try {
    ...
    if (response.status === 422) {
      ...
      const sectionListTopElements = document.querySelector('.section_list_top .count-text');
...
function renderProviderProfiles(data) {
  ...
  data.items.forEach(async (provider) => {
    ...
    const selectedList = provider.specialties.filter((s) => selectedSpecialties.includes(s.slug));
    ...
    spTagContainer.querySelectorAll('.is-last-tag').forEach((el) => el.remove());

    let remaining = orderedSpecialties.length - Math.min(visibleLimit, provider.specialties.length);
    if (remaining > 0) {
    ...
    getAvailabilitySlots(provider.provider_id).then(function (response) {
      ...
      element.find('.rd-card_dates-wr .rd-card_date-link:not(.is-clone)').remove();

      response.slots.slice(0, 7).forEach((slot) => {
        const dateClone = element.find('.rd-card_dates-wr .rd-card_date-link.is-clone').clone(true);
  ...
  setTimeout(function () {
    const countElement = document.querySelector('.section_list_top .count-text');
    if (countElement) {
      countElement.querySelector('span').textContent = data.total.toLocaleString('en-US');
      if (zipInput.value != '') {
        $('.count-state-pretext').show();
        countElement.querySelector('.count-state-text').textContent = '' + zipInput.value;
      } else {
...
function updateVisibleTags() {
  ...
  document.querySelectorAll('.dietitian-item:not(.is-clone)').forEach((item) => {
    const spTags = item.querySelectorAll('.sp-tags .sp-tag:not(.is-last-tag)');

    spTags.forEach((tag, index) => {
      tag.classList.toggle('hide', index >= visibleLimit);
    });

    const mobileELement = item.querySelector('.sp-tags .sp-tag.is-last-tag.is-mobile');
    const desktopELement = item.querySelector('.sp-tags .sp-tag.is-last-tag.is-desktop');
    if (isMobile) {
...
function checkBackgroundText() {
  $('.rd-approach-text').each((index, element) => {
    ...
    const $toggleBtn = $(element).closest('.dietitian-item').find('.background-view-more');
...
$(document).on('click', '.background-text', function () {
  const viewMoreElement = $(this).closest('.dietitian-item').find('.background-view-more');

  if (viewMoreElement.text() == 'View more') {
    viewMoreElement.closest('.dietitian_card').find('.background-text').removeClass('is-close');
    ...
    viewMoreElement.closest('.dietitian_card').find('.background-text').addClass('is-close');
...
$(document).on('click', '.background-view-more', function () {
  if ($(this).text() == 'View more') {
    $(this).closest('.dietitian_card').find('.background-text').removeClass('is-close');
    $(this).text('View less');
    /*  if (window.innerWidth <= 767) {
      $(this)
        .closest(".dietitian_card")
    ...
    $(this).closest('.dietitian_card').find('.background-text').addClass('is-close');
...
$(document).on('click', '.rd-card_date-link', function () {
  ...
  const providerTitle = $(this).closest('.dietitian-item').find('.provider-title').text();
  let formattedDate = '';
...
window.addEventListener('message', function (event) {
  ...
  } else if (event.data.source === 'availability-app' && event.data.type === 'slotConfirmed') {
    const urlParams = new URLSearchParams(window.location.search);
    const insurance = urlParams.get('insurance');
    const specialties = urlParams.getAll('specialties');
...
function formatDateLabel(dateString) {
  ...
  if (slotDay.getTime() === tomorrow.getTime()) return { slot, dateStr: 'Tomorrow' };
...
async function getRating(provider_id, gateOn) {
  try {
    ...
    if (response.status === 200) {
      const data = await response.json();
      if (gateOn && data.rating !== undefined && data.rating.average !== undefined) {
        ...
        if (data.rating.average > 0) {
          ...
          reviewEle.find('.cr-r-amount .cr-r-amount-underline').text(data.rating.total_submissions);
...
function setRating(rating, provider_id) {
  ...
  reviewEle.find('.rd-r-stars-wr .rd-r-star').each(function (index) {
    ...
    $(this).find('.star-fill-rect').attr('clip-path', `url(#starClip-${provider_id}-desktop)`);
    $(this)
      .find('.star-fill-rect')
      .closest('svg')
      .find('clipPath')
...
function initSearchElement() {
  ...
  window.addEventListener('pageshow', function (e) {
    ...
    if (isBack && !sessionStorage.getItem("b_once")) {
      sessionStorage.setItem("b_once", "1");
      setTimeout(() => location.reload(), 500); // last resort
    } */
    ...
    if (!isResultForm) {
      ...
      if (navigated) {
        ...
        $(
          ...
          $('#search-insurance-label').text('Insurance*').removeClass('is-active');
  ...
  function preFillSearchAfterBack() {
    ...
    try {
      let preSearchValue = {};
      try {
        preSearchValue = JSON.parse(sessionStorage.getItem('preSearchValue') || {});
      } catch (error) {
        preSearchValue = {};
      }
      ...
      if (preSearchValue?.state) {
        if (preSearchValue?.stateInputText !== undefined && preSearchValue?.stateInputText != '') {
      ...
      if (preSearchValue?.specialties && preSearchValue?.specialties.length > 0) {
  ...
  $('#insurance-dropdown .insurance-wrapper .filter-item-radio').change(function (e) {
    $('#insurance-dropdown .insurance-wrapper  .filter-item-radio').removeClass('is-active');
    $(this).addClass('is-active');

    const labelText = $(this).find('.radio-dropdwn-label').text();
    searchFilters.insurance = $(this).find('input[type="radio"]').attr('data-value');
  ...
  $('#search-insurance .filter-dropdown_list .filter_radio-button-field').change(function (e) {
    $('#search-insurance .filter-dropdown_list .filter_radio-button-field').removeClass(
      'is-active'
    );
    $(this).addClass('is-active');
  ...
  $('#search-specialty-dropdown .save').click(function (e) {
    ...
    $('#search-specialty-dropdown .specialties-field input[type="checkbox"]').each(
      (index, checkboxInput) => {
        const isChecked = $(checkboxInput).prop('checked');
        if (isChecked) {
    ...
    if (selectedSpecialties.length > 0) {
      $('#search-specialties-label').text(selectedSpecialtiesText.join(', ')).addClass('is-active');
    } else {
      $('#search-specialties-label').text('Specialties').removeClass('is-active');
    }

    $('#search-specialty-dropdown-mobile .filter_checkbox-field input[type="checkbox"]').each(
      (index, checkboxInput) => {
...
function applySearch() {
  ...
  if (!hasError) {
    ...
    // const newUrl = `${window.location.pathname}?${params.toString()}`;
    const newUrl = `https://signup.faynutrition.com/quiz?${params.toString()}`;
...
function applyPreselectedValues() {
  ...
  if (insurance && insurance.length > 0) {
    $('.find-data-wr .filter-dropdown_list.is-insurance  .filter_radio-button-field').each(
      function (e) {
        if ($(this).find('input').attr('data-value') == insurance) {
          $(this).addClass('is-active');
          $(this).find('.filter_radio-button').addClass('w--redirected-checked');
          const labelText = $(this).find('.filter_radio-label').text();
          $('.find-data-wr #insurance-label').text(labelText).addClass('is-active');
        }
      }
    );
  } else {
    ...
    labels.forEach((label) => {
      specialties.forEach((specialty) => {
        ...
        if (labelValue === specialty.trim().toLowerCase()) {
          ...
          label.parentNode.querySelector('input[type="checkbox"]').checked = true;
  ...
  if (modalities.length > 0) {
    $('.filter-dropdown_list.is-modalities .w-dyn-item, #modality-collection .w-dyn-item').map(
      function () {
        const index = $.inArray(
    ...
    document.querySelector('.modalities-number').innerHTML = `(${modalities.length})`;
  ...
  if (availabilityType) {
    ...
    radioInput.closest('.filter_radio-button-field:not(.is-mobile)').addClass('is-active');
    radioInput
      .closest('.filter_radio-button-field')
...
function resetSpecialtyHeroSearch() {
  ...
  $(
    ...
    const input = $(element).find('input[type="checkbox"]');
    const dataValue = input.attr('data-value');
    if (searchFilters.specialties && searchFilters.specialties.includes(dataValue)) {
      input.prop('checked', true);
      $(element).find('.w-checkbox-input').addClass('w--redirected-checked');
      const text = $(element).find('.w-form-label').text().trim();
      if (!selectedSpecialtiesText.includes(text)) {
...
async function loadRatings() {
  ...
  const promises = $('#dietitian-items-container .dietitian-item').map(async function (i, element) {
    const providerId = $(element).attr('id');
    const ratingData = await getRating(providerId, gateOn);
    ratingsData.push(ratingData?.rating !== undefined ? ratingData.rating : {});
  });
  ...
  if (insurance) {
    const insuranceElement = $('.filter-dropdown_list.is-insurance').find(
      '.filter_radio-button-field input[name="Insurance"][data-value="' + insurance + '"]'
    );
  ...
  if (specialties && specialties.length > 0) {
    for (let i = 0; i < specialties.length; i++) {
      ...
      specialtiesText.push(
        specialtyElement.closest('.filter_checkbox-field').find('.filter_checkbox-label').text()
      );
    }
  }
  ...
  let eventData = {
    ...
    Location: state,
    Specialties: specialtiesText && specialtiesText.length > 0 ? specialtiesText.join(', ') : null,
    Modalities: modalitiesText && modalitiesText.length > 0 ? modalitiesText.join(', ') : null,
  };
  ...
  if (ratingsData.length > 0) {
    ...
    $.each(ratingsData, function (_, d) {
      ...
      let count =
        d?.total_submissions && d.total_submissions != null ? parseInt(d.total_submissions) : 0;
    ...
    const averageRatingCount = totalRecords > 0 ? totalRatingsCount / totalRecords : 0;
    ...
    eventData = {
      ...
      Location: state,
      Specialties:
        specialtiesText && specialtiesText.length > 0 ? specialtiesText.join(', ') : null,
      Modalities: modalitiesText && modalitiesText.length > 0 ? modalitiesText.join(', ') : null,
    };
  }
  ...
  availableTags.forEach((tag) => {
    const isVisible = !!(tag.offsetWidth || tag.offsetHeight || tag.getClientRects().length);
    if (isVisible) {
